...
<style>
    /* ===== Video Banner (namespaced) ===== */
    .video-banner{
        position:relative; overflow:hidden; border-radius:16px;
        height:clamp(320px, 45vw, 640px);       /* cao linh hoạt, không chiếm toàn màn */
        background:#000 center/cover no-repeat; /* dùng làm poster fallback nếu cần */
    }
    .video-media{ position:absolute; inset:0; overflow:hidden; }
    .video-iframe{
        position:absolute; top:50%; left:50%; transform:translate(-50%,-50%);
        pointer-events:none;                     /* cho phép click nút/CTA phía trên */
        /* Mặc định: rộng hơn để phủ kín theo chiều ngang */
        width:177.78%; height:100%;
    }
    /* Nếu màn hình cao (hẹp hơn 16:9), tăng chiều cao để phủ kín */
    @media (max-aspect-ratio: 16/9){
        .video-iframe{ width:100%; height:177.78%; }
    }

    /* Lớp tối để nội dung nổi bật */
    .video-scrim{
    ...
    .video-content{
        ...
        max-width:960px; padding:28px; /* bạn có thể đổi căn giữa/ trái… */
    }
    .video-content h2{ margin:0; font-size:clamp(22px, 4vw, 40px); line-height:1.15; }
    .video-content p{ margin:0; opacity:.9; font-size:clamp(14px, 2vw, 18px); }
    .video-content .btn{
        display:inline-block; margin-top:14px; padding:10px 16px; border-radius:10px;
        background:#0d6efd; color:#fff; text-decoration:none; font-weight:700;
    }

    /* Optional: full-bleed trên mobile nếu muốn cao hơn */
    @media (max-width: 576px){
        .video-banner{ height:clamp(260px, 55vw, 420px); border-radius:12px; }
    }



    /* === FULL-BLEED banner bất chấp container/padding === */
    .section_slider { padding:0; }

    /* Fallback: dùng vw */
    .section_slider .video-banner{
        position:relative; overflow:hidden; border-radius:0;
        width:100vw; max-width:100vw;
        margin-left:calc(50% - 50vw); margin-right:calc(50% - 50vw);
        height:clamp(360px, 56.25vw, 820px);   /* 56.25vw = 16:9 full width */
        background:#000 center/cover no-repeat;
    }

    /* Ưu tiên dvw/svw để tránh lệch do scrollbar & thanh URL mobile */
    @supports (width: 100dvw){
        .section_slider .video-banner{
            width:100dvw; max-width:100dvw;
            margin-left:calc(50% - 50dvw); margin-right:calc(50% - 50dvw);
            height:clamp(360px, 56.25dvw, 820px);
        }
    }

    /* Iframe fill kiểu cover (không để lộ viền đen) */
    .section_slider .video-media{ position:absolute; inset:0; overflow:hidden; }
    .section_slider .video-iframe{
        position:absolute; top:50%; left:50%; transform:translate(-50%,-50%);
        width:177.78%; height:100%;            /* phủ ngang cho màn hình rộng */
        pointer-events:none;
    }
    @media (max-aspect-ratio:16/9){
        .section_slider .video-iframe{ width:100%; height:177.78%; } /* phủ dọc khi màn hình cao */
    }

    /* Nếu layout ngoài có padding/overflow, nới ra để không bị cắt */
    .section_slider .container, .section_slider .row{ padding:0; overflow:visible; }
...
<section class="section_slider video-full">
    <!-- Banner Video -->
    <div class="video-banner"
         data-youtube="{{ $config->link_youtube }}"
         data-poster="/path/to/fallback.jpg">
        <div class="video-media" aria-hidden="true">
            <iframe class="video-iframe" title="Video banner" src=""
...
{{--            <h2>TIÊU ĐỀ BANNER</h2>--}}
{{--            <p>Mô tả ngắn về chiến dịch / sản phẩm.</p>--}}
...
<script>
    (function(){
        // Lấy ID từ mọi dạng link YouTube (watch?v=, youtu.be/, embed/)
        function getYouTubeId(urlOrId){
            if(!urlOrId) return "";
            const m = String(urlOrId).match(/(?:youtu\.be\/|v=|embed\/)([A-Za-z0-9_-]{6,})/);
            return m ? m[1] : urlOrId; // nếu đã là ID thì trả về luôn
        }

        document.querySelectorAll('.video-banner').forEach(banner=>{
            const id = getYouTubeId(banner.dataset.youtube || "");
            const iframe = banner.querySelector('.video-iframe');

            // Poster fallback nếu muốn (ảnh sẽ nằm dưới iframe)
            if (banner.dataset.poster){
                banner.style.backgroundImage = `url("${banner.dataset.poster}")`;
            }
    ...
    @foreach ($categoryProductSpecials as $categoryProductSpecial)
    @if ($categoryProductSpecial->products->count() > 0)
        <div class="section_product_new section_product">
            <div class="container">
                <div class="block-title">
                    <h2><a href="{{ route('front.show-product-category', ['categorySlug' => $categoryProductSpecial->slug]) }}"
                           title="{{ $categoryProductSpecial->name }}">{{ $categoryProductSpecial->name }}</a></h2>
                    <div class="block-view">
                        <a href="{{ route('front.show-product-category', ['categorySlug' => $categoryProductSpecial->slug]) }}"
                    ...
                    <div class="col-right col-lg-12 col-12">
                        <div class="block-product relative">
                            <div class="product-new-swiper swiper-container">
                                <div class="swiper-wrapper">
                                    @foreach ($categoryProductSpecial->products as $item)
                                        <div class="swiper-slide">
